...
class GetModel{
		...
		if(count($linkToArray)>1){
			...
			foreach ($linkToArray as $key => $value) {
				...
				if($key > 0){
					...
					$linkToText .= "AND ".$value." LIKE '%".$searchArray[$key]."%' ";
				}
			}
	...
	static public function getDataRange($table,$select,$linkTo,$between1,$between2,$orderBy,$orderMode,$startAt,$endAt, $filterTo, $inTo){
		...
		if($filterTo != null && $inTo != null){
			...
			foreach ($filterToArray as $key => $value) {
				...
				$filter .= " AND ".$value." LIKE '%".$inToArray[$key]."%'";
	...
	/*=============================================
	Peticiones GET personalizadas
	=============================================*/

	static public function getCustom($table, $custom){

		if($table != Connection::customKey()){
