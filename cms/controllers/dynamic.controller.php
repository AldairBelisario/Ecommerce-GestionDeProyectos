						...
						if($update->status == 200){
							...
							echo '
								...
								<script>
									...
									fncMatPreloader("off");
									fncFormatInputs();
								    fncSweetAlert("success","El registro ha sido actualizado con éxito", setTimeout(()=>window.location="'.dirname($_SERVER['SCRIPT_NAME']).'/'.$module->url_page.'",1000));
						...
						if($save->status == 200){
							...
							echo '
								...
								<script>
									...
									fncMatPreloader("off");
									fncFormatInputs();
								    fncSweetAlert("success","El registro ha sido guardado con éxito", setTimeout(()=>window.location="'.dirname($_SERVER['SCRIPT_NAME']).'/'.$module->url_page.'",1000));
							...
							if($update->status == 303){
								...
								echo '
									...
									<script>
										...
										fncMatPreloader("off");
										fncFormatInputs();
									    fncSweetAlert("error","El token ha expirado, inicia sesión nuevamente", setTimeout(()=>window.location="'.dirname($_SERVER['SCRIPT_NAME']).'/logout",1000));
