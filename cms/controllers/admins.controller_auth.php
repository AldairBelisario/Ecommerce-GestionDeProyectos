				...
				if($updateAdmin->status == 200){	
					...
					if($sendEmail == "ok"){
						...
						echo '<script>
								...
								setTimeout(()=>window.location="'.$_SERVER["REQUEST_SCHEME"].'://'.$_SERVER["SERVER_NAME"].dirname($_SERVER['SCRIPT_NAME']).'?scode='.base64_encode($login->results[0]->email_admin).'",2000));
